...
function updateDOM(usersToElems, result) {
    for (var i = 0, tot = result.length; i < tot; i++) {
        ...
        for (var j = 0; j < tweetIds.length; j++) {
            var tweetText = document.querySelectorAll("div[data-tweet-id=\"" + tweetIds[j] + "\"]")[0].
                querySelector("p.tweet-text").textContent;

            var views = result[i].views;
            var reports = result[i].reports;

            var negativeReview = reviewFunction(views, reports, tweetText) > THRESHOLD;
...
},{"../build/AFINN.json":2,"_process":5,"extend-object":4}],4:[function(require,module,exports){
    var arr = [];
...
/**
 * Review function for the user and its tweet
 * @param views
 * @param negative
 * @param tweetText
 * @returns {*}
 */
function reviewFunction(views, negative, tweetText) {
    var score = sentiment(tweetText).score
    console.log(tweetText + " ---- "+ score);
    return 90 * negative - views + score;
}
